...
# -----------------------------------------------------------
# Training function for Strategy 2: Center-Seeking Reward
# -----------------------------------------------------------
def train_strategy_b(strategy_name="strategy2_center_seeking"):
    # Create folders for this new strategy
    os.makedirs("weights", exist_ok=True)
    os.makedirs("experiments/strategy2", exist_ok=True)
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            # Penalty based on the cart's distance from the center of the track (state[0]).
            cart_position = state[0]
            position_penalty = 0.1 * abs(cart_position)
            modified_reward = reward - position_penalty

            # Push the experience with the MODIFIED reward to the replay buffer
    ...
    df.to_csv(f"experiments/strategy2/results.csv", index=False)
    print("Training log saved to experiments/strategy2/results.csv")

    env.close()


if __name__ == "__main__":
    train_strategy_b(strategy_name="strategy2_center_seeking")
